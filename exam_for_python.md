...
## Тип 2: 28677, 15124, 26974, 15618, 15970, 18614, 55589, 48423, 15787, 85678, 18578, 81786, 17320, 48450, 40718, 73828, 45236, 33504, 64932, 64887, 72587
## Тип 8: 3697, 78064, 55625, 7667, 17328, 27539, 18491, 37143, 59745, 10473, 60250, 64938, 9796, 27009, 15822, 28546, 15795, 3700, 3237, 13486, 59741
# Решения задач:
...
c = 0
for p in product (sorted('ВИНТ'), repeat=5):
    c += 1
    if c == 1019:
        break
print(p)
...
```
### Ответ: xzyw
> ### Пояснение: ПОСЛЕДНИЙ это `W`, т.к у него 0, первый строка это [1 1 0 0], вторая строка [0 1 0 0], тогда второй столбец занимает `Z`, первая строка [1 0 1 0], третья получается это `Y`, первая `X`
...
```
### Ответ: yxwz
> ### Пояснение: третий столбец занимает `W`, первый столбец `Y`, если учесть что строки иду сверху вниз, второй `X`, четвертый `Z`
...
for x in range(0, 2):
    for y in range(0, 2):
        for z in range(0, 2):
            for w in range(0, 2):
                if not(((w <= (not x)) == (z <= y)) and (y or w)): # для 0
                    print(x, y,z, w)
```
```
print('x y z w')
for x in range(0, 2):
    for y in range(0, 2):
        for z in range(0, 2):
            for w in range(0, 2):
                if ((w <= (not x)) == (z <= y)) and (y or w): # для 1
                    print(x, y,z, w)
```
|x|y|z|w|
|:-:|:-:|:-:|:-:|
...
|0|0|0|1|
### Ответ: xwyz
> ### Пояснение: первая строка это [1 1 0 1] для #0, вторая имеет два ноля и единицы, этому подходит 0101, 0110, 1100, но во всех столбцах есть нули, значит у нас не может быть три единицы, значит вторая строка [0 1 1 0], а третья [0 0 0 1]. первый столбец занимает `X`, второй `W`, третий `Y` и четвертый `Z`
...
for x in range(0, 2):
    for y in range(0, 2):
        for z in range(0, 2):
            for w in range(0, 2):
                if (x <= (y == w)) and (y == (w <= z)): # для 1
                    print(x, y, z, w)
```
### Ответ: yxwz
> ### Пояснение: как минимум известно две раскрытые, значит надо чтобы наше решение подходило этим условиям. Исходя из этого  #для 0 берем [0 0 1 0]. #для 1, первой строкой [0 1 1 0], у второй строки три 0, поэтому подходит [0 0 0 1] для условий. Второй столбец `X`с тремя 0, первый `Y`, третий `W`, четвертый `Z`   
...
print(ci)
```
> **Пояснение:** ```r = oct(n)[2:]``` перевод в восьмиричную систему и начниается с числа 2. ```for c in range(len(r)-1):``` указвается диапазон и провеяет на уникальность если условие не подходит он его отсеивает. Далее условие прерывания если одно из двух он проходит идет дальше. ``Break`` нужен что он не проверял то что не в нашем диапазоне.
### Ответ: 180
...
from itertools import *
c = 0
for p in product (sorted('НИКОЛАЙ'), repeat=4):
    if p[0] != 'Й' and any (x in p for x in 'ИОА'):
        c += 1
print(c)
```
## Второй вариант
    ...
    for y in a:
        for z in a:
            for w in a:
                    ...
                    r = x + y + z + w
                    if len(set(s))==4:
                        s = s.replace('Н', '*').replace('С', '*').replace('Т', '*')
                        r = r.replace('А', '*').replace('Я', '*')
                        if (s.count('*')<3 and s.count('**')==0) and (r.count('*')<3 and r.count('**')==0):
                            c += 1
print(c)
```
### Ответ: 24
...
```
### Ответ: wzxy
> ### Пояснение: т.к первая строка имеет три ноля, значит [1 0 0 0], вторая две нули, ей подходит [1 0 0 1], учитываем что в одном из столбцов сразу три ноля, поэтому третья строка это [1 0 1 1], последний столбец занимает `Y`, первый `W`, вторую `Z` и третью `X`
...
for x in range(0, 2):
    for y in range(0, 2):
        for z in range(0, 2):
            for w in range(0, 2):
                if ((x <= (z <= w)) and (z <= (y == (not w)))) == False:
                    print(x, y, z, w)

```
### Ответ: zxwy
> ### Пояснение: первой строкой является [0 0 1 0], т.к мы занем что `Z` первый птому что у него все 1, вторая строка [1 0 1 0] т.к z все 1, значит второй столбец тоже 1, значит второй это `X`, третья это [1 1 1 0], исходя того что только у w три ноля, значит третий это `W`, и четвертый это `Y`
## Тип 8 59741
