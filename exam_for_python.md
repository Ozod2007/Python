...
```
### Ответ: wzxy
> Пояснение: т.к первая строка имеет три ноля, значит [1 0 0 0], вторая две нули, ей подходит [1 0 0 1], учитываем что в одном из столбцов сразу три ноля, поэтому третья строка это [1 0 1 1], последний столбец занимает `Y`, первый `W`, вторую и третью
